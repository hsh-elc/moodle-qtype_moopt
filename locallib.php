...
function get_task_xml_file_from_filearea($question) {
    $fs = get_file_storage();
    $files = $fs->get_area_files($question->contextid, 'question', PROFORMA_TASKXML_FILEAREA, $question->id, "", false);
...
function save_task_and_according_files($question) {
    ...
    if ($taskzipfilename != null) {
        ...
        $file = $fs->get_file($question->context->id, 'question', PROFORMA_ATTACHED_TASK_FILES_FILEAREA, $question->id, '/', $taskzipfilename);
        $newfilerecord = array(
            'component' => 'question',
